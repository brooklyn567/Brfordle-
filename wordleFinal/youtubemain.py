...
def check_guess(guess_to_check):
    ...
    for i in range(5):
        ...
        if lowercase_letter in CORRECT_WORD: # Check if the letter is in the correct word
            if lowercase_letter == CORRECT_WORD[i]:  # If the letter is in the correct position
                guess_to_check[i].bg_color = GREEN # indicates green
                for indicator in indicators:   # update color indicators 
                ...
                if not game_decided:
                    game_result = "W"
            else: # If the letter is in the correct word but in the wrong position
                ...
                game_result = ""
                game_decided = True
        else:  # If the letter is not in the correct word
            guess_to_check[i].bg_color = GREY # updates indicator color 
            for indicator in indicators:
                if indicator.text == lowercase_letter.upper():
...
def reset():
    ...
    SCREEN.blit(BACKGROUND, BACKGROUND_RECT)
    guesses_count = 0
    CORRECT_WORD = random.choice([word for word in WORDS if len(word) == 5])
    guesses = [[]] * 5
    current_guess = []
    current_guess_string = ""
    game_result = ""
...
while True: # Check if the game result is not an empty string
    if game_result != "":
        play_again()
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN:
                if game_result != "":
                    reset()
                else: # Check if the current guess has 5 letters and is a valid word
                ...
                if len(current_guess_string) > 0:
                    delete_letter()
            else:   # Checks for other letter key that could be pressed 
